...
using tiny::add_into;
using tiny::remove_from;
using tiny::delete_all;
using tiny::delete_not_null;
using tiny::replace_all;
...
ConditionNode* ConditionMgr::build_integer_node(tree_node_t* node)
{
    ...
    int value = tiny::str_to<int>(node->value);
...
bool QueryMgr::insert_into(tree_node_t* node)
{
    ...
    vector<pair<string, string>> data = tiny::vector_make_pair(name_list, value_list);
