...
{
    vector<string> tokens = tiny::tokenize(buf, " \t");
    if (tokens.empty())
        return CMD_OK;
...
CmdState cmd_readfile(const char* file_name)
{
    ...
    tiny::UpdateTo<const char*> update_name(&parser_file_name, file_name);
    tiny::UpdateTo<int> update_lineno(&parser_file_lineno, 0);
